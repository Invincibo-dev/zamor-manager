...
const getDashboard = async (_req, res, next) => {
  try {
    ...
    // CURDATE() uses the MySQL server's local timezone — avoids UTC offset issues.
    const _svcFallback = () => ({
      natcash:  { depot: { count: 0, total: 0 }, retrait: { count: 0, total: 0 }, transfert: { count: 0, total: 0 } },
      recharges: { natcom: { count: 0, total: 0 }, digicel: { count: 0, total: 0 } },
    });

    const serviceStats = await (async () => {
      try {
        const [natByType] = await mysqlPool.query(
          `SELECT service_type, COUNT(*) AS count, COALESCE(SUM(amount), 0) AS total
           FROM natcash_transactions
           WHERE DATE(created_at) = CURDATE()
           GROUP BY service_type`
        );
        const [rchByCompany] = await mysqlPool.query(
          `SELECT company, COUNT(*) AS count, COALESCE(SUM(amount), 0) AS total
           FROM recharges
           WHERE DATE(created_at) = CURDATE()
           GROUP BY company`
        );

        const result = _svcFallback();
        for (const row of natByType) {
          if (result.natcash[row.service_type] !== undefined) {
            result.natcash[row.service_type] = { count: Number(row.count), total: Number(row.total) };
          }
        }
        for (const row of rchByCompany) {
          if (result.recharges[row.company] !== undefined) {
            result.recharges[row.company] = { count: Number(row.count), total: Number(row.total) };
          }
        }
        return result;
      } catch (err) {
        console.error("[dashboard] serviceStats error:", err.message);
        return _svcFallback(); // always return non-null so frontend section renders
      }
    })();
