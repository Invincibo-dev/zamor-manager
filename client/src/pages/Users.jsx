...
function Users() {
  ...
  const handleCreateSeller = async (event) => {
    ...
    try {
      ...
      setCreateForm({ name: "", email: "", password: "", role: "vendeur" });
      setToast({
        type: "success",
        message: `Compte créé : ${data.user.email} (${data.user.role})`,
      });
    } catch (requestError) {
      setToast({
        type: "error",
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 sm:gap-6">
        <section className="rounded-[28px] bg-white p-4 shadow-[0_12px_30px_-18px_rgba(15,23,42,0.22)] sm:p-5 lg:p-6">
          <div className="mb-5">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Cree un compte utilisateur en quelques secondes.
            </p>
          </div>
        ...
        <section className="rounded-[28px] bg-white p-4 shadow-[0_12px_30px_-18px_rgba(15,23,42,0.22)] sm:p-5 lg:p-6">
          <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Gere les comptes sans toucher au reste.
              </p>
            </div>

            <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
              <span className="font-semibold text-slate-950">{sellers.length}</span>{" "}
              compte{sellers.length !== 1 ? "s" : ""} actif{sellers.length !== 1 ? "s" : ""}
            </div>
          </div>
