    ...
    <main className="bg-white text-black">
      <div className="mx-auto w-[58mm] bg-white px-[2.5mm] py-[2.5mm] font-mono text-[10px] leading-[1.2] sm:w-[80mm] print:w-[58mm]">
        ...
        <div className="flex items-center justify-between gap-3 text-[11px] font-bold">
          ...
          <span>
            {receipt.devise === "USD"
              ? `${formatAmount(receipt.total_general)} USD`
              : `${formatAmount(receipt.total_general)}G`}
          </span>
        </div>
        {receipt.devise === "USD" && receipt.taux_change ? (
          <div className="mt-[1px] text-right text-[9px] text-black/60">
            {`= ${formatAmount(Number(receipt.total_general) * Number(receipt.taux_change))}G (taux ${formatAmount(receipt.taux_change)})`}
          </div>
        ) : null}
