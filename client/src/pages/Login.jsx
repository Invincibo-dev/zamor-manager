...
function Login() {
  ...
  if (existingUser) {
    return (
      <Navigate
        to={existingUser.role === "admin" || existingUser.role === "gestionnaire" ? "/dashboard" : "/create-sale"}
        replace
      />
    );
  }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      navigate(data.user.role === "admin" || data.user.role === "gestionnaire" ? "/dashboard" : "/create-sale", {
        replace: true,
      });
    } catch (submitError) {
