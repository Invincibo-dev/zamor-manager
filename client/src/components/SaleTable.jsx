...
const buildWhatsAppUrl = (sale) => {
  const date = new Date(sale.date).toLocaleDateString("fr-FR");
  const total = Number(sale.total_general || 0).toLocaleString("fr-FR", {
        ...
        {sales.map((sale) => (
          ...
          >
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:gap-3">
              ...
              <a
                href={buildWhatsAppUrl(sale)}
                target="_blank"
                rel="noreferrer"
                className="flex w-full items-center justify-center rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3.5 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100"
              >
                WhatsApp
              </a>
              {onPrint ? (
                <button
                  ...
                  className="col-span-2 w-full rounded-2xl bg-slate-900 px-4 py-3.5 text-sm font-semibold text-white transition hover:bg-slate-800"
                >
                  Imprimer
                </button>
              ) : null}
